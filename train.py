...
# unmark it if you need to generate your own dataset for training.
    ...
    # write_to_tfrecords({"source":np.asanyarray([i.T for i in source])[np.random.choice(range(train_num),train_num)],
    #                "target":np.asanyarray([i.T for i in source])},"Def_train_{}_{}_{}.tfrecords".format(clas,def_level,train_num))

    print(np.asanyarray([i.T for i in a.target_list])[np.random.choice(range(train_num),train_num)])
    print(np.asanyarray([i.T for i in a.target_list]))

    print(np.asanyarray(source))
