...
def apply_tps_deformation(arr: np.ndarray, num_of_points: int=3) -> np.ndarray:
    # Create grid of control points
    if arr.shape[-1] == 3:
        control_points = np.asarray([[x, y, z] for x in [i for i in range(num_of_points)]
                                                for y in [i for i in range(num_of_points)]
                                                 for z in [i for i in range(num_of_points)]])

    elif arr.shape[-1] == 2:
        control_points = np.asarray([[x, y] for x in [i for i in range(num_of_points)]
                                                for y in [i for i in range(num_of_points)]])
    # Randomly perturb control points
    deformations = np.random.uniform(-0.3, 0.3, arr.shape[-1])
    target_points = np.stack([np.squeeze(control_points[i] + deformations)
...
class PointRegDataset(Dataset):
    def __init__(self,
        ...
        if clas==0:
            source = np.loadtxt('EM.txt')[::10] / 1000
        ...
        if clas==1:
            ...
            source_temp=np.asarray(sc).reshape(-1,2)

            source = np.zeros((91, 3))
            source[:, :2] = source_temp

            source = source / 10

        if clas==2:
            source=np.loadtxt('bunny.txt')
            source=source[::5, :3]# ).reshape(-1,2)
            # source=source[:256,:]
...
#             source=(source/source.max()).reshape(-1,2)
            ...
            targ = apply_tps_deformation(source)
            # print(targ.shape)
            # targ=Tps_trans(np.tile(np.expand_dims(source,0),[3,1,1]),
            #  np.tile(np.expand_dims(theta,0),[3,1]))
            # # print(targ.shape)
            # targ=targ[0]
            targc=targ
            ...
            if self.outlier_ratio!=0:
                ...
                print('Outlier shape: ', targ.shape)
            ...
            if self.missing_points!=0:
                ...
                print('Missing shape: ', targ.shape)
